...
def count_sets(cards):
    ...
    for i in range (0, len(combos)):
        ...
        if combos[i][0] != combos[i][1]:
            if combos[i][1] != combos[i][2]:
                if combos[i][0] != combos[i][2]:
                    unique = True
        if unique == False:
            raise ValueError("The cards are not all unique")

        if is_set(int(combos[i][0]), int(combos[i][1]), int(combos[i][2])):
            number_of_sets += 1
